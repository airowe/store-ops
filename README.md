...
The loop runs in your editor, on your machine, with your credentials. The 33
...
| Path | What |
|------|------|
| **`skills/` + `lib/`** | The **free OSS agent plugin** — 33 skills + the engine (Python, 200 tests). Runs in any coding agent; Claude Code also has a one-command installer. |
...
| `asc-screenshot-write-lane` | Put an approved run's screenshot strip on the editable version through ShipASO — ids resolved, per-shot ledger, **stops before submit**. |
...
audit → research → optimize → push → verify loop, both stores, all 33 skills —
...
|---|---|---|
| The 33 skills + full loop | ✅ all of it | ✅ same engine |
