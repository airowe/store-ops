...
> Note: this branch's B slice is guard-safe — it does NOT touch `portfolio.tsx`, so `noIapPurchasePath` stays green. The paywall component exists and is tested but is not yet mounted in a screen (that's B4).
