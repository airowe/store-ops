...
- [x] **B4 (merged #432).** Paywall mounted on the tier-gated screens via a `TierGate` wrapper (replacing the "sell nothing" gate) + Restore Purchases reachable from the paywall. `noIapPurchasePath.test.mjs` was rewritten: its premise is flipped (IAP is now permitted; steering to the web is the violation), and it now asserts a 402 screen mounts `TierGate` and that no screen sends the user to the web to buy/manage.
- [x] **B4a — purchase-screen disclosures (merged #431, issue #430).** The paywall discloses, per subscription: price **per period** (`renewalSentence` in `subscriptionPeriod.ts`, omitting the rate when the store reports no period), an **auto-renewal + where-to-cancel** clause, what the tier **includes** (the store `description`, 3.1.2(c)), and **Terms of Use** + **Privacy Policy** links (`legalUrls.ts`, rendering no control when a URL is unset so it never links to a 404). 2.3.7-safe (no "free"/price text baked into UI copy).

> Note (Schedule 2 caveat, still open): the itemized purchase-screen requirements derive from **Schedule 2 of the Apple Developer Program License Agreement**, which was not fetchable to quote verbatim (only 3.1.2(c) and 5.1.1(i) were confirmed from the public guidelines). Read Schedule 2 before the D-resubmit to confirm the list is complete.
...
- [x] **C4 (follow-up, this branch).** `readiness.ts` now audits `REVENUECAT_WEBHOOK_AUTH` (`revenuecat_webhook_auth`) and the three `REVENUECAT_PRODUCT_*` ids (`revenuecat_products`) as `warn` checks, so `/health` flags when the IAP webhook is unconfigured (it 503s) or a tier can't be mapped. Tested in `readiness.spec.ts`.
