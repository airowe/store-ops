...
| **Hosted MCP as the front door** (distribution) | ✅ **merged + live 2026-09-05** — keyless `claude mcp add shipaso` works in production (12 tools listed; keyed tools answer with mint instructions; invalid key → 401); listed in the MCP Registry as `io.github.airowe/shipaso` 0.1.0 (status active) | Found 2026-09-05: `POST /mcp` had been live since #93 while `docs/prd/mcp-server.md` said "deferred"; the route sat behind login. Loop scope + exit criteria: PR #528 |
...
| Screenshot + preview-footage capture (agent-driven) | 🔵 **first run done locally 2026-09-05** — all 4 planned shots captured for real at 1320x2868, manifest honest, 0 missing; found the workflow could never have run (Debug build, no Metro) → fixed to Release in its PR. CI dispatch still needs the `ANTHROPIC_API_KEY` **Actions** secret (**yours**) | `.github/workflows/capture-shots.yml` + `marketing/screenshots/capture/PLAN.md`; raw captures feed `scripts/render-shipshots.py` |
