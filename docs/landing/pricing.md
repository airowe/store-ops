...
| Startup | $19.99/mo | 10 | Yes | The recurring loop for a mid-sized portfolio. |
| Scale | $64.99/mo | 50 | Yes | Portfolio across many apps, with the fleet roll-up view. |
...
- **Measured numbers only.** An unmeasured value renders as "—", never a guess
  and never zero. There is no keyword volume or difficulty score, because no
  honest measured source exists for one.
- **Approval is the terminus.** ShipASO never publishes to a store on its own.
  Every change waits for an explicit human approval, on every plan.
