...
export function auditReadiness(env: Env): ReadinessReport {
  ...
  // ── warns: email delivery (Resend) ────────────────────────────────────────
  const emailOk = isSet(env.RESEND_API_KEY) && isSet(env.RESEND_FROM);
  checks.push({
    name: "email_delivery",
    ok: emailOk,
    severity: "warn",
    detail: emailOk
      ...
      : "No email provider set (RESEND_API_KEY/RESEND_FROM) — magic-link emails are only logged, not delivered.",
  });
